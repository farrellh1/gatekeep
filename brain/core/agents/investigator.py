...
def run(state: BrainState, config: RepoConfig, reader: RepoReader | None = None) -> BrainState:
    ...
    RunContext is built per event so reader-using checks share a single index. A
    caller may inject a snapshot-hydrated reader so the checks run against a frozen
    index instead of a clone; when None, the reader is built from the event's clone.
    """
    ctx = RunContext(state.event, reader=reader)
