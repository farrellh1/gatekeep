...
from core.schemas import BrainState, Finding, NormalizedEvent, Verdict
...
def test_auto_gate_allows_close_above_threshold(monkeypatch):
    ...
    out = resp_mod.run(
        _state("slop", 0.95, [HIGH_FAIL]), RepoConfig(mode="auto-gate", threshold=0.85)
    )
...
def test_auto_gate_strips_close_below_threshold(monkeypatch):
    monkeypatch.setattr(resp_mod, "draft_comment", lambda st: "x")
    out = resp_mod.run(
        _state("slop", 0.70, [HIGH_FAIL]), RepoConfig(mode="auto-gate", threshold=0.85)
    )
